...
export function getDeptCodeFromPostal(postal: string): string {
  if (/^97[1-6]/.test(postal)) return postal.slice(0, 3)
  if (postal.startsWith('200') || postal.startsWith('201')) return '2A'
  if (postal.startsWith('20')) return '2B'
  return postal.slice(0, 2).replace(/^0(\d)$/, '0$1') // keep leading zero
}
