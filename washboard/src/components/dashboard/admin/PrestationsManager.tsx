...
const EMPTY_FORM = { name: '', price: '', duration_minutes: '', vehicle_types: [] as string[] }
...
export default function PrestationsManager({ services: initial }: { services: Service[] }) {
  ...
  async function add() {
    ...
    const res = await fetch('/api/services', {
      ...
      body: JSON.stringify({ ...form, price: Number(form.price), duration_minutes: Number(form.duration_minutes) }),
    })
  ...
  function startEdit(svc: Service) {
    setEditId(svc.id)
    setForm({ name: svc.name, price: String(svc.price), duration_minutes: String(svc.duration_minutes), vehicle_types: svc.vehicle_types })
    setShowAdd(false)
  }
      ...
      <div className="bg-blue-50 dark:bg-blue-950/30 border border-blue-200 dark:border-blue-800 rounded-xl p-4 space-y-3">
        <div className="grid grid-cols-2 gap-3">
          <div className="col-span-2">
            ...
            <input type="text" value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} placeholder="Lavage intérieur + extérieur" className={inputClass} />
          </div>
          <div>
            <label className="block text-xs font-medium text-slate-600 dark:text-slate-400 mb-1">Prix (€)</label>
            <input type="number" min="0" value={form.price} onChange={e => setForm(f => ({ ...f, price: e.target.value }))} placeholder="80" className={inputClass} />
          </div>
          <div>
            <label className="block text-xs font-medium text-slate-600 dark:text-slate-400 mb-1">Durée (min)</label>
            <input type="number" min="15" step="15" value={form.duration_minutes} onChange={e => setForm(f => ({ ...f, duration_minutes: e.target.value }))} placeholder="90" className={inputClass} />
          </div>
        </div>
        <div>
        ...
        <div className="flex gap-2 pt-1">
          <button type="button" onClick={onSave} disabled={loading || !form.name || !form.price || !form.duration_minutes}
  ...
  return (
    <div className="space-y-3">
      ...
      {services.map(svc => (
        <div key={svc.id}>
          {editId === svc.id ? (
            <ServiceForm onSave={() => update(svc.id)} onCancel={() => { setEditId(null); setForm(EMPTY_FORM) }} />
          ) : (
            <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm p-4 flex items-center gap-4">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
                  {svc.price}€ · {svc.duration_minutes} min · {svc.vehicle_types.join(', ') || 'Tous véhicules'}
                </p>
              </div>
      ...
      {showAdd && (
        <ServiceForm
          onSave={add}
          onCancel={() => { setShowAdd(false); setForm(EMPTY_FORM); setError(null) }}
        />
      )}
