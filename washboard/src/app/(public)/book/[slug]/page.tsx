...
export default async function BookingPage({ params }: Props) {
  ...
  // service-role absente/invalide, RLS mal configurée...) retombe sur `?? []`
  // plus bas et fait apparaître TOUS les créneaux comme libres côté client —
  // déjà vécu en prod (double-réservation, congés ignorés). Voir TODO.md.
  if (bookingsError) logger.error('book.bookings.fetch_failed', { washerId: washer.id }, bookingsError)
  if (unavailError) logger.error('book.unavailabilities.fetch_failed', { washerId: washer.id }, unavailError)
